...
train_df = df[df["Race"].isin(["Australia", "China"])]
# Test on current race
test_df = df[df["Race"] == "Miami"].copy()

if test_df.empty:
    raise ValueError("❌ No data found for Miami in season_dataset.csv. Run merge_dataset.py first.")
