...
# PATHS
MON_BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
MON_RAW_PATH = os.path.join(MON_BASE_DIR, "data/raw")
MON_PROCESSED_PATH = os.path.join(MON_BASE_DIR, "data/processed/dataset.csv")
MON_PREV_GP_DATA = os.path.abspath(os.path.join(MON_BASE_DIR, "../miami_gp/data/processed/dataset.csv"))
...
# LOAD
fp1 = pd.read_csv(f"{MON_RAW_PATH}/fp1.csv")
fp2 = pd.read_csv(f"{MON_RAW_PATH}/fp2.csv")
fp3 = pd.read_csv(f"{MON_RAW_PATH}/fp3.csv")
qualifying = pd.read_csv(f"{MON_RAW_PATH}/qualifying.csv")
results = pd.read_csv(f"{MON_RAW_PATH}/race_results.csv")
...
# HISTORY
if os.path.exists(MON_PREV_GP_DATA):
    prev_df = pd.read_csv(MON_PREV_GP_DATA)
...
os.makedirs(os.path.dirname(MON_PROCESSED_PATH), exist_ok=True)
df.to_csv(MON_PROCESSED_PATH, index=False)
print("✅ Monaco dataset RANKED (Stability Fixed)")