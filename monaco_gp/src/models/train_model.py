...
train_df = df[df["Race"].isin(["Australia", "China", "Miami","Japan","Canada"])]
# Test on current race
test_df = df[df["Race"] == "Monaco"].copy()

if test_df.empty:
    raise ValueError("❌ No data found for Monaco in season_dataset.csv. Run merge_dataset.py first.")
